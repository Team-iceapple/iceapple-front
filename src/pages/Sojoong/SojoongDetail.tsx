...
function transformHtml(raw: string, stylesRef: typeof styles): string {
    const doc = document.implementation.createHTMLDocument("sojoong");
    const root = doc.createElement("div");
    root.innerHTML = raw ?? "";
    root.querySelectorAll("img").forEach((img) => {
        img.setAttribute("loading", "lazy");
        img.classList.add(stylesRef.responsiveImg);
    });
    root.querySelectorAll("iframe, video, object, embed").forEach((el) => {
        (el as HTMLElement).classList.add(stylesRef.responsiveMedia);
    });
    root.querySelectorAll<HTMLAnchorElement>("a[href$='.pdf']").forEach((a) => {
        const href = a.getAttribute("href") || "";
        if (!href) return;
        if (a.closest(`.${stylesRef.pdfEmbed}`)) return;
        const wrap = doc.createElement("div");
        wrap.className = stylesRef.pdfEmbed;
        const iframe = doc.createElement("iframe");
        iframe.setAttribute("src", href);
        iframe.setAttribute("title", "PDF 미리보기");
        iframe.setAttribute("loading", "lazy");
        const caption = doc.createElement("div");
        caption.className = stylesRef.pdfCaption;
...
function collectHeadings(root: HTMLElement | null): HTMLElement[] {
    if (!root) return [];
    return Array.from(root.querySelectorAll("h1, h2, h3, .section, [data-section='true']")).filter(
...
const SojoongDetail = ({ id, onClose }: Props) => {
    ...
    useEffect(() => {
        const candidates = [
            `${apiBase}notice/sojoong/${encodeURIComponent(id)}`,
            `${apiBase}notice/api/sojoong/${encodeURIComponent(id)}`
        ];
        (async () => {
                ...
                try {
                    ...
                    const html =
                        mapped.has_attachment && isEmptyHtml(mapped.content)
                            ? ""
                            : transformHtml(mapped.content ?? "", styles);
    ...
    useEffect(() => {
        ...
        const onScroll = () => {
            ...
            setProgress(p);
            setShowTop(el.scrollTop > 0); // ← 여기만 변경
        };
    ...
    const linkUrl = (() => {
        ...
        if (!raw) return `${window.location.origin}/sojoong/${notice.id}`;
        try {
